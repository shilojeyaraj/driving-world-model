"""Render the world model's DREAM (V1). Condition on a few real frames, then roll the PRIOR
forward with the true actions (sample=False -> the prior mean) and DECODE each latent into a
predicted frame. Save ground-truth vs. dreamed frames side by side -- the iconic world-model
visualization: you watch what the model thinks will happen.

Usage:  python -m scripts.dream_video [checkpoint.pt]
        (no checkpoint -> trains a small image-mode world model inline)
"""
import os, sys
...
def dream_video(ckpt=None, image_size=32, wm_steps=500, context=5, horizon=15,
                out_dir="runs/dream", seed=1):
    from models.world_model import WorldModel

    if ckpt:
        from utils import load_models
        cfg, wm, _, _ = load_models(ckpt)
        context = min(context, cfg.seq_len - horizon)
    else:
        cfg = get_config(obs_type="image", env="dummy", image_size=image_size,
                         deter_dim=128, stoch_dim=32, hidden_dim=128,
                         seq_len=context + horizon + 4, max_episode_steps=200, batch_size=16)
    ...
    # One trajectory: condition on `context`, dream `horizon` with the TRUE actions (prior mean).
    np.random.seed(seed)
    eval_buf = _collect(cfg, 2000, seed=seed)
    batch = {k: torch.as_tensor(v) for k, v in eval_buf.sample(4).items()}
    obs, actions = batch["obs"], batch["action"]
    B = obs.shape[0]
    wm.eval()
    with torch.no_grad():
        emb = wm.encoder(obs[:, :context].reshape(B * context, *obs.shape[2:])).reshape(B, context, -1)
        state = wm.rssm.observe(emb, actions[:, :context], wm.rssm.initial_state(B, obs.device))["state"]
        ia = actions[:, context - 1: context - 1 + horizon]
        feat = wm.rssm.imagine(ia, state, sample=False)["feat"]
        pred = wm.decoder(feat.reshape(B * horizon, feat.shape[-1]))["obs"].reshape(B, horizon, *obs.shape[2:])
    true = obs[:, context: context + horizon]

    # Save: a montage PNG (row 0 = truth, row 1 = dream) and an animated GIF (truth | dream).
    import imageio.v2 as imageio
    os.makedirs(out_dir, exist_ok=True)
    b = 0
    t = true[b].cpu().numpy(); p = pred[b].cpu().numpy()
    ...
    montage = np.concatenate([row_true, sep_h, row_pred], axis=0)
    imageio.imwrite(os.path.join(out_dir, "dream_montage.png"), montage)
