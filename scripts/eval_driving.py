...
def _fmt(s):
    if s.get("n", 0) == 0:
        return "no episodes"
    return (f"route {s['route_completion']:.0%}  success {s['success_rate']:.0%}  "
            f"crash {s['crash_rate']:.0%}  off-road {s['off_road_rate']:.0%}  "
            f"return {s['mean_return']:+.1f}  steps {s['mean_steps']:.0f}  (n={s['n']})")
