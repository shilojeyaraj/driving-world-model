...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{../runs/direct_bc/by_scene.png}
\caption{
  Per-geometry route completion and off-road rate for
  \texttt{policy\_boosted.pt} on held-out maps. Straight/curve/intersection
  are near-IDM level; roundabout is the residual failure mode, reduced from
  60\% to 30\% off-road by scene-targeted augmentation.
}
\label{fig:byscene}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{../runs/milestones.png}
\caption{
  Project arc: route completion across all milestones from random baseline
  through DAgger. The world-model latent bottleneck (2\%) is bypassed by
  direct BC (24\%), which is improved via recovery data (39\%) and
  scene-targeted augmentation (64\% on roundabouts). DAgger matches the
  augmented baseline at iteration 1 but regresses by iteration 3 due to
